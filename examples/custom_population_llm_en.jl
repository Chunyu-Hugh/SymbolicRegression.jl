"""
Custom Initial Population + LLM Iterative Optimization Example
...
3. Based on your analysis, generate $new_needed new candidate expressions with the following requirements:
   ...
   - Use operations such as +, -, *, /, safe_pow(·,·), cos
...
function sanitize_expression(expr::String)
    # LLM-returned expressions should already be in x1, x2 format, just need to convert ^ to safe_pow
    sanitized = expr
...
function run_llm_guided_search(; num_rounds::Int=3, iterations_per_round::Int=20, select_count::Int=5)
    println("=" ^ 80)
    println("Custom Initial Population + LLM Iterative Optimization Example")
    println("=" ^ 80)
