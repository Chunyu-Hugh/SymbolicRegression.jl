"""
Custom Initial Population + LLM Iterative Optimization Example (Euclidean Distance Formula)
...
This example is specifically designed to discover the Euclidean distance formula: d = sqrt((x1-x2)^2 + (y1-y2)^2 + ...)
...
function create_llm_prompt(
    ...
    return """
You are a symbolic regression and geometry/distance calculation expert. We have completed round $round/$total_rounds of search.

Goal: Discover the Euclidean distance formula from data. The data contains coordinates of two points (x1, y1, x2, y2), and the target is to calculate the Euclidean distance between the two points.
...
1. Analyze the geometric meaning, types (e.g., distance formula, sum of squares, square root, etc.), and their relationship to Euclidean distance.
2. Select $select_count expressions that are most worth keeping and exploring further (indices start from 1).
3. **Important: Based on your analysis, suggest new operators that need to be added**. If the current operators are insufficient to express the Euclidean distance formula, please explicitly suggest which new operators are needed (choose from the system-supported operator library, e.g., sqrt is important for distance calculation).
4. Based on your analysis, generate $new_needed new candidate expressions with the following requirements:
   ...
   - Expressions must use variable names x1, y1, x2, y2 (representing x-coordinate of first point, y-coordinate of first point, x-coordinate of second point, y-coordinate of second point, respectively)
   - No variable names other than x1, y1, x2, y2 are allowed
   - Consider the mathematical form of Euclidean distance: sqrt((x1-x2)^2 + (y1-y2)^2)
...
- Euclidean distance formula typically requires sqrt function
"""
end
...
function run_llm_guided_search_euclidean_distance(;
    ...
    println("=" ^ 80)
    println("Custom Initial Population + LLM Iterative Optimization Example (Euclidean Distance Formula)")
    println("=" ^ 80)

    # Generate Euclidean distance data
    # Data format: X contains [x1, y1, x2, y2], y is the Euclidean distance between the two points
    n_samples = 100
    X = randn(Float64, 4, n_samples)  # 4 features: x1, y1, x2, y2
    # Calculate true Euclidean distance: sqrt((x1-x2)^2 + (y1-y2)^2)
